...
| Path | Current proof |
|---|---|
| Buyer — `/` | Mobile-first discovery by date, pickup/delivery, postcode, occasion, style, and budget; bookable seeded results; product context; a single-product basket; separate buyer/recipient inputs; transparent demo total; simulated order request. |
| Buyer tracking — `/order/:id` | Seller-confirmation state, next action, timeline, visibly labelled order messaging, fulfilment progress, and fee summary. |
| Seller — `/seller` | Deadline-led action queue, accept/decline and fulfilment transitions, new-order pause/resume, product pause/restore, capacity view, and simulated payout summary. |
...
- One availability calculation is reused by catalogue, product detail, and order submission for the seeded model. It checks seller/product state, lead time, method, date, capacity, delivery sector, fee, and budget.
- Order creation conditionally reserves capacity in a database batch, and order, transition, and message retries use idempotency keys.
...
- Commercial, operational, payment, and payout states are separate. Acceptance simulates capture; decline simulates void and releases reserved capacity.
...
|---|---|---|
| Core marketplace — FR-C01–C07 | Both fulfilment methods, shared eligibility logic, conditional capacity reservation, one-product/one-seller orders, immutable snapshots, Singapore timezone, and idempotency on key order writes. | Add a real cart and atomic 15-minute checkout hold; retain capacity through `accept_by`; add expiry/recovery workers; make **every** command idempotent; prove race behaviour, transaction boundaries, and notification/payment side effects under load. |
